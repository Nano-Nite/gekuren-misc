...
func SetupMiscRoute(app *fiber.App, apiVersion string) {
	...
	app.Post(miscBaseURL+"/education-level", func(c fiber.Ctx) error {
		payload := new(model.SearchPayload)
		tenantUUID, _, err := helper.ValidateRequest(c)
		if err != nil {
			return helper.ReturnResponse(c, fiber.StatusUnauthorized, "Missing or invalid authentication data", nil, err)
		...
		data, stats, err := helper.SearchEducationLevel(tenantUUID.String(), *payload)
		if err != nil {
			return helper.ReturnResponse(c, fiber.StatusBadRequest, "Failed to search users", nil, err)
		}
		return helper.ReturnResponse(c, fiber.StatusOK, "success", map[string]any{"data_statistic": stats, "result": data}, nil)
	...
	app.Post(miscBaseURL+"/position", func(c fiber.Ctx) error {
		payload := new(model.SearchPayload)
		tenantUUID, _, err := helper.ValidateRequest(c)
		if err != nil {
			return helper.ReturnResponse(c, fiber.StatusUnauthorized, "Missing or invalid authentication data", nil, err)
		...
		data, stats, err := helper.SearchPosition(tenantUUID.String(), *payload)
		if err != nil {
			return helper.ReturnResponse(c, fiber.StatusBadRequest, "Failed to search users", nil, err)
		}
		return helper.ReturnResponse(c, fiber.StatusOK, "success", map[string]any{"data_statistic": stats, "result": data}, nil)
	...
	app.Post(miscBaseURL+"/employee_status", func(c fiber.Ctx) error {
		payload := new(model.SearchPayload)
		tenantUUID, _, err := helper.ValidateRequest(c)
		if err != nil {
			return helper.ReturnResponse(c, fiber.StatusUnauthorized, "Missing or invalid authentication data", nil, err)
		...
		data, stats, err := helper.SearchEmployeeStatus(tenantUUID.String(), *payload)
		if err != nil {
			return helper.ReturnResponse(c, fiber.StatusBadRequest, "Failed to search users", nil, err)
		}
		return helper.ReturnResponse(c, fiber.StatusOK, "success", map[string]any{"data_statistic": stats, "result": data}, nil)
